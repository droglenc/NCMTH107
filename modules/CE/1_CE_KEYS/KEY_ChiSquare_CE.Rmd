...
<li>If you have raw data (as in the pupularity and polls exercises, then use `chisq.test()` to perform all of the required calculations. There should not be any need to do extra work outside of R when you have the raw data.</li>
<li>When using `xtabs()` to prepare the observed table for `chisq.test()`, you don't need to worry about using `factor()` with `levels=` unless you want to change the order of the levels. In fact, that is never needed as the chi-square values will be the same no matter the order of the levels.</li>
</ul>
</div>
...
<ol>
  ...
  <li>A chi-square test is required because (i) a categorical response variable with four levels (race/ethnicity) from (ii) two groups or populations (sex) was recorded.</li>
  <li>This is an observational study without obvious randomization.</li>
