...
1. The residual for an observed IBI score of 30 and a percent of impervious surface of 10% is 30-`r formatC(predict(lm1,data.frame(imp=10)),format="f",digits=1)`=`r formatC(20-predict(lm1,data.frame(imp=10)),format="f",digits=1)`.
1. The IBI will change by -25 slopes or `r formatC(-25*cfs[2],format="f",digits=4)` if the percentage of impervious surface is decreased by 20%. In other words the IBI will increase by `r formatC(-25*cfs[2],format="f",digits=4)` units.
