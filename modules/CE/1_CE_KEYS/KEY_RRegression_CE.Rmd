...
1. The best-fit line is **fawnrec=`r formatC(coef(lm1)[2],format="f",digits=6)`$\times$RIFA+`r formatC(coef(lm1)[1],format="f",digits=3)`**.
1. The slope indicates that for every increase of one for the RIFA index the index of fawn recruitment will decrease by **`r formatC(-1*coef(lm1)[2],format="f",digits=6)`**, on average.
1. The intercept indicates that if the RIFA index was zero, then the index of fawn recruitment would be **`r formatC(coef(lm1)[1],format="f",digits=3)`**, on average.
1. If the RIFA index increased by 500 then the predicted index of fawn recruitment would decrease by 500 slopes or **`r formatC(-500*coef(lm1)[2],format="f",digits=3)`**.
...
1. The predicted index of fawn recruitment if the RIFA index is 1700 is **`r formatC(predict(lm1,data.frame(rifa=1700)),format="f",digits=3)`**.
1. The residual for an individual with a RIFA index of 2200 and a fawn recruitment index of 0.3 is 0.3-`r formatC(predict(lm1,data.frame(rifa=2200)),format="f",digits=3)`=**`r formatC(0.3-predict(lm1,data.frame(rifa=2200)),format="f",digits=3)`**. Thus, this individual would have a lower fawn recruitment than the average for locations with a RIFA index of 2200.
