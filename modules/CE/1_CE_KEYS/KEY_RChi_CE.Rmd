...
1. It appears that the distributions of goals by students does not differ by grade. Analysis of the row percentage table (results of `percTable()` below) suggests that grades are the primary goal for students in each grade.
...
1. A goodness-of-fit test is required because a single categorical variable (habitat use) from a single group or population (this bear) was measured and the proportions are being compared to a theoretical distribution in the null hypothesis.
1. This is an observational study with randomly selected times for observation.
