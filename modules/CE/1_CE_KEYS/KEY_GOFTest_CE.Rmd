...
tbls(name="TurkeyExpTable","Expected frequency of turkeys (assuming that Ho is true) by habitat in the fall.")
tbls(name="TurkeyObsTable","Observed frequency of turkeys by habitat in the fall.")
...
<li>All cells in the expected frequency table (`r tbls("WaterExpTable",display="cite")`) were greater than 5; the assumptions for a goodness-of-fit test have been met. [Note that these expected values equal 107/4 to represent no preference in choice of water.]</li>
...
<li>It appears that there is a preference for water type by the students. Further examination suggests that the students least-preferred the tap water.</li>
...
#t.obs <- c(Aspen=61,Meadow=9,Pine=807,OakSpruce=1)  # Summer
t.obs <- c(Aspen=32,Meadow=16,Pine=449,OakSpruce=5) # Fall
...
<ol>
<li>&alpha;=0.05.</li>
<li>H<sub>A</sub>: "There is a preference by the turkeys for habitat (i.e., the distribution of turkeys will NOT follow the (theoretical) distribution of available habitats)" and H<sub>0</sub>: "There is NOT a preference by the turkeys for habitat (i.e., the distribution of turkeys will follow the (theoretical) distribution of available habitats)".</li>
...
<li>All cells in the expected frequency table (`r tbls("TurkeyExpTable",display="cite")`) were greater than 5; the assumptions for a goodness-of-fit test have been met. [Note that these expected values are equal to 878 total observations times the proportions of available habitat to represent no preference in choosing habitat types by the turkeys.]</li>

`r tbls("TurkeyExpTable")`

```{r echo=FALSE}
round(t.chi$expected,1)
```
...
<!---- this was for spring
<li>&chi;<sup>2</sup>=$\frac{(61-45.3)^{2}}{45.3}$ + $\frac{(9-89.2)^{2}}{89.2}$ +$\frac{(807-734.8)^{2}}{734.8}$ +$\frac{(1-8.8)^{2}}{8.8}$=`r formatC(t.chi$statistic,format="f",digits=3)` with `r t.chi$parameter` df (`r tbls("TurkeyResults",display="cite")`).</li>
---->

<li>&chi;<sup>2</sup>=$\frac{(32-25.9)^{2}}{25.9}$ + $\frac{(16-51.0)^{2}}{51.0}$ +$\frac{(449-420.1)^{2}}{420.1}$ +$\frac{(5-5.0)^{2}}{5.0}$=`r formatC(t.chi$statistic,format="f",digits=3)` with `r t.chi$parameter` df (`r tbls("TurkeyResults",display="cite")`).</li>
...
<!---- this was for spring
<li>It appears that the turkeys show a habitat preference in spring. Further analysis suggests that the turkeys showed a strong avoidance of meadows, a moderate avoidance of Oak/Spruce habitats, and a moderate preference for aspen and pine habitats.</li>
---->

<li>It appears that the turkeys show a habitat preference in fall. Further analysis suggests that the turkeys showed a strong avoidance of meadows and a moderate preference for aspen and pine habitats.</li>
<li>Not required for a goodness-of-fit test.</li>
</ol>
...
<li>&chi;<sup>2</sup>=$\frac{(23-81.496)^{2}}{81.496}$ + $\frac{(96-200.400)^{2}}{200.400}$ +$\frac{(134-180.360)^{2}}{180.360}$ +$\frac{(293-289.912)^{2}}{289.912}$+$\frac{(297-204.408)^{2}}{204.408}$+$\frac{(380-243.152)^{2}}{243.152}$+$\frac{(113-136.272)^{2}}{136.272}$=`r formatC(j.chi$statistic,format="f",digits=4)` with `r j.chi$parameter` df.</li>
