...
<strong>Note:</strong>
<ul>
<li>Here and in the next question, make sure to list all of the factors, list levels seperately for each of the factors, and list the number of replicates as the number of individuals PER treatment (not all of the individuals in the experiment). For the R work you don't need to use the code with the square brackets, just split the random numbers up into as many groups as you have treatments.</li>
</ul>
</div>
...
<strong>Note:</strong>
<ul>
<li>The randomization in this example is fairly non-typical and is completely dependent on the way the researchers set up this experiment. In other words, focus on the previous examples for a more general method of randomizing individuals for the treatments.</li>
</ul>
</div>

1. This is an experiment because the researcher is imparting treatments (ball types and bats were chosen specifically).
1. The response variable is the distance that the ball was hit.
1. Distance is a quantitative, continuous variable because a distance can always be found between any two distances.
1. The two factors are brand of ball and type of bat.
1. There are two levels of ball brand and three levels of bat type.
1. There are six treatments (unique combinations of two balls and three bats).
1. There are four replicates (balls of each brand by each type of bat).
1. A "replicate" (or "individual") in this experiment is a "hit."
