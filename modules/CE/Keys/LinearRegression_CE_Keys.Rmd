...
1. This prediction should not be made as an age of 90 is outside the observed results for this variable (`r figs("VisionFitPlot")`) and is, thus, an extrapolation.
...
1. I don't have any strong concerns, though the data look very slightly nonlinear and very slightly heteroscedastic (`r figs("VisionFitPlot")`).

```{r VisionFitPlot, echo=FALSE}
...
1. This prediction should not be made as an index count of 20 is outside the observed results for this variable (`r figs("FishingFitPlot")`) and is, thus, an extrapolation.
...
1. The data appears to be roughly linear but this is difficult to tell based on the wide scatter of points.  My concern, though, is that the data appear to be somewhat heteroscedastic (`r figs("FishingFitPlot")`).

```{r FishingFitPlot, echo=FALSE}
fitPlot(lm1,pch=21,bg="gray70",
