...
<strong>Note:</strong> 
<ul>
<li>The value in the `mu=` argument is the specific value that appears in H<sub>0</sub> in Step 2. It is NOT the value of x&#772; from Step 6.</li>
</ul>
</div>
...
<ol>
  <li>&alpha;=0.01.</li>
  <li>H<sub>A</sub>:&mu;<5.6 and H<sub>0</sub>:&mu;=5.6, where &mu; is the mean pH for all rainwater collections at the Big Meadows Station in the Shenandoah National Park, VA.</li>
...
```
  ...
  <li>One is 99% confident that the mean pH level is less than `r formatC(ph.t$conf.int[2],format="f",digits=2)` (`r tbls("pH",display="cite")`), which further indicates acid rain at this site.</li>
</ol>
...
## Sea Level Rise Rate
```{r SeaLevelCalc, echo=FALSE, results='hide', fig.show='hide'}
...
<ol>
  <li>&alpha;=0.01.</li>
  <li>H<sub>A</sub>: &mu;>0 and H<sub>0</sub>:&mu;=0, where &mu; is the mean sea level rise rate for all ecosystems. [Note that a mean of 0 means no change.]</li>
  ...
  <li>t=$\frac{0.059-0}{\frac{0.135}{\sqrt{23}}}$=`r formatC(t,format="f",digits=3)` with `r df` df.</li>
  <li>p-value=`r kPvalue(pv,include.p=FALSE,latex=FALSE)`.</li>
  ...
  <li>One is 95% confident that the mean sea level rise rate is greater than $0.059-2.508\frac{0.135}{\sqrt{23}}$=`r formatC(lcb,format="f",digits=3)`, which further indicates that the mean rise rate is not greater than 0 and the sea level has not risen significantly over the study period.</li>
</ol>
