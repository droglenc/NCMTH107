...
<ol>
<li>&alpha; = 0.05.</li>
<li>H<sub>A</sub>: &mu;<sub>bf</sub>-&mu;<sub>lp</sub> &neq; 0 and H<sub>0</sub>: &mu;<sub>bf</sub>-&mu;<sub>lp</sub> = 0, where &mu; is the mean reduction in LDL, bf is befizal, and lp is lipanthyl.</li>
</ol>

<div class="alert alert-info">
<ul>
  <li>The hypotheses can also be written as -- H<sub>A</sub>: &mu;<sub>bf</sub>&neq;&mu;<sub>lp</sub> and H<sub>0</sub>: &mu;<sub>bf</sub>=&mu;<sub>lp</sub> -- because if two values are thought to be equal then the difference between those values would be equal to 0 and if they are thought to differ than the difference between the two would be diffrent from zero.</li>
...
<ul>
  ...
  <li>The independence characteristics is the most difficult to write in my opinion. Basically what you have to describe is that there is not direct connection between individuals in one group and indviduals in the other group. A lack of independence in this example could be that the same people were in both drug groups (think of a before-after study) or that siblings were split into the two groups (so one sibling was always "connected" genetically to their sibling in the other group).</li>
  <li>If the two groups have different sample sizes (as is the case here) the you automatically know that there is independence because for individuals to be connected there would have to be the same number per group.</li>
</ul>
</div>

<ol start="4">
  <li>This study is largely experimental because the experimenter decided which individuals received which drug treatment.</li>
</ol>

<div class="alert alert-info">
<ul>
  <li>This is the same as what you did for the 1-sample tests. However, you will start seeing more "experiments" in the 2-sample tests.</li>
</ul>
</div>

<ol start="5">
  <li>The assumptions are met because (i) the samples are independent (as described above), (ii) n<sub>bf</sub>+n<sub>lp</sub> = 13+12 = 25 &gt; 15 and the two sample distributions (histograms shown on the handout) are not obviously strongly skewed, and (iii) the variances appear to be equal (Levene's p-value = 0.2414 &gt; &alpha;).</li>
</ol>  
  
<div class="alert alert-info">
<ul>
  <li>If the combined sample size is less than 30 then you need to assess the skewness of the histograms. As with the 1-sample t-test, this assumption is no longer refers to the population distributions.</li>
  <li>The "equal variances" assumption is assessed with a Levene's Test. The hypotheses for the Levene's test are H<sub>0</sub>: &sigma;<sup>2</sup><sub>bf</sub>=&sigma;<sup>2</sup><sub>lp</sub> (i.e., variances are equal) and H<sub>A</sub>: &sigma;<sup>2</sup><sub>bf</sub>=&neq;&sigma;<sup>2</sup><sub>lp</sub> (i.e., variances are unequal). Thus, if the p-value from the Levene's test is greater than &alpha; the variances are considered equal, the assumption is met, and you can continue with the 2-sample t-test.</li>
...
<div class="alert alert-info">
<p>The following steps perform a number of calculations using summary statistics from the table provided on the handout. I have placed those values with their corresponding symbols in the bullets below for convenience. Note that the subscripts correspond to the drug treatments as defined in the hypotheses above.</p>
<ul>
  <li>n<sub>bf</sub>=`r n1`; n<sub>lp</sub>=`r n2`</li>
  <li>x&#772;<sub>bf</sub>=`r xbar1`; x&#772;<sub>lp</sub>=`r xbar2`</li>
  <li>s<sub>bf</sub>=`r s1`; s<sub>lp</sub>=`r s1`.</li>
  <li>s<sup>2</sup><sub>bf</sub>=`r s1`<sup>2</sup>=`r formatC(s1^2,format="f",digits=4)`; s<sup>2</sup><sub>lp</sub>=`r s1`<sup>2</sup>=`r formatC(s2^2,format="f",digits=4)`</li>
</ul>
</div>
...
<div class="alert alert-info">
  <p>The next step requires two intermediate calculations as shown below. Note that the values used in these equations were identified above.</p>
  <ul>
