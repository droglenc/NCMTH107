...
tbls(name="hurr3","Results from the chi-square test for examining the distribution of hurricanes by strength and time period categories.")
...
colnames(obs) <- c("1&2","3","4&5")
( hurr.chi <- chisq.test(obs,correct=FALSE) )
```

<ol>
  <li>&alpha;=0.10.</li>
  <li>H<sub>O</sub>: "The distribution of hurricantes into the strength categories is the same for both time periods" versus H<sub>A</sub>: "The distribution of hurricantes into the strength categories is NOT the same for both time periods"</li>
...
```
  ...
  <li>The &chi;<sup>2</sup> test statistic is `r formatC(hurr.chi$statistic,format="f",digits=1)` with `r formatC(hurr.chi$parameter,format="f",digits=0)` df (`r tbls("hurr3",display="cite")`).</li>
