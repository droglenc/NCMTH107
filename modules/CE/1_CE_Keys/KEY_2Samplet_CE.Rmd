...
<li>Note all H<sub>A</sub> are "not equals" for a 2-sample t-test (and these exercises were examples of that). In the first exercise the goald was to determine if the homemade compost had less germinated flowers. As the label for homemade was alphabetically second the H<sub>A</sub> is a "greater than" (i.e., bought-homemade would be positive numbers if homemade was smaller).</li>
...
<li>When the raw data are available, as in the Delivery Times exercise, then load those data into R and use `t.test()` to perform the required calcuulations for Steps 6 (you still need to subtract the means), 7, 8, and 11. When entering the data yourself, <a href="http://derekogle.com/NCMTH107/resources/FAQ/FAQs/stacked-data.html">make sure it is stacked</a>.</li>
</ul>
</div>
...
( tstar <- distrib(0.9,type="q",distrib="t",df=df,lower.tail=FALSE) )
( lcb <- stat+tstar*se )
```
...
<li>An experimental study was conducted without obvious random selection of the containers with soil, but with randomization allocation (placement of containers in the greenhouse).</li>
...
<li>x&#772;<sub>b</sub>-x&#772;<sub>h</sub> = `r xbar1`-`r xbar2` = `r stat`. The pooled sample variance is,
      \[s_{p}^{2}=\frac{(18-1)4.33^{2}+(10-1)3.21^{2}}{18+10-2} = 15.826 \]
       ...
       \[ SE_{\bar{x}_{did}-\bar{x}_{didnot}}=\sqrt{15.826\left(\frac{1}{18}+\frac{1}{105} \right)} = 1.569  \]</li>
...
<li>2-sample t-test because (i) two groups/populations are considered (Bosnia and Persian Gulf), (ii) a quantitative variable (delivery time) was recorded, and (iii) the individuals in the groups are independent (there is no known connection between all deliveries to the two wars).</li>
<li>An observational study without clear randomization of the two samples.</li>
<li>The assumptions are met because (i) the samples are independent (as described above), (ii) n<sub>b</sub>+n<sub>g</sub> = 9+9 = 18 &gt; 15 and the two sample distributions (`r figs("DeliveriesHist",display="cite")`) are not obviously strongly skewed (also told to assume that the distributions are not strongly skewed), and (iii) the variances appear to be equal (Levene's p-value = `r  kPvalue(del.lev$"Pr(>F)"[1],include.p=FALSE)` &gt; &alpha;).</li>
...
<li>I am 95% confident that the mean delivery time for Bosnia was more than `r formatC(-1*del.t$conf.int[2],format="f",digits=1)` days faster than for the Persian Gulf War (`r tbls("Deliveries",display="cite")`). This further supports that the mean delivery time was faster with the new system.</li>
</ol>
...
( m.t <- t.test(mhg~grp,data=df,var.equal=TRUE) ) # mu=0, alt="two-sided" are defaults
```

<ol>
  <li>&alpha;=0.05.</li>
  <li>H<sub>0</sub>:&mu;<sub>i</sub>=&mu;<sub>r</sub> (or &mu;<sub>i</sub>-&mu;<sub>r</sub>=0) and H<sub>A</sub> is &mu;<sub>i</sub>&ne;&mu;<sub>r</sub> (or &mu;<sub>i</sub>-&mu;<sub>r</sub>&ne;0), where &mu; is the mean methyl mercury level, i corresponds to the "impacted" group, and r corresponds to the "reference" group.</li>
  <li>A two-sample t-test is required because a quantitative response variable (methyl mercury level) was measured on two groups ("impacted" and "reference" sites) that were independent (no connection between sites at the two locations).</li>
  <li>The data appear to be part of an observational study without clear randomization.</li>
  <li>The assumptions are not quite met but we will continue anyway. The two groups appear to be independent. The variances appear to be equal because the p-value from the Levene's test is "large" (i.e., `r kPvalue(m.lt[1,3],latex=FALSE)`>0.05; `r tbls("mussels1",display="cite")`). The sample size (n<sub>i</sub>+n<sub>r</sub>=`r formatC(m.t$parameter+2,format="f",digits=0)`) is not &ge;15 and there is not enough data to make useful histograms. Therefore, the test statistic computed below may not follow a t-distribution with n<sub>i</sub>+n<sub>r</sub>-2=`r formatC(m.t$parameter,format="f",digits=0)` df.</li>
...
```
  ...
  <li>The statistic is x&#772;<sub>i</sub>-x&#772;<sub>r</sub> = `r formatC(m.t$estimate[1],format="f",digits=3)` - `r formatC(m.t$estimate[2],format="f",digits=3)` = `r formatC(m.t$estimate[1]-m.t$estimate[2],format="f",digits=3)` (`r tbls("mussels2",display="cite")`).</li>
...
```
  ...
  <li>The t test statistic is `r formatC(m.t$statistic,format="f",digits=3)` with `r formatC(m.t$parameter,format="f",digits=0)` df (`r tbls("mussels2",display="cite")`).</li>
  ...
  <li>I am 95% confident that the difference in mean methyl mercury between mussels from the impacted and reference sites is between `r formatC(m.t$conf.int[1],format="f",digits=3)` and  `r formatC(m.t$conf.int[2],format="f",digits=3)`. This further supports (because 0 is in the confidence region) that the mean methyl mercury does not differ between mussels from the two sites.</li>
</ol>
