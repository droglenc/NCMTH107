...
<li>The x-axis (explanatory) variable is <strong>increased</strong> by 1 unit when describing the slope. If the slope is positive then the y-axis (response) variable increases by the slope amount. If the slope is negative then the y-axis (response) varible decreases by the slope amount (with the negative sign removed).</li>
...
<li>When computing the residual, make sure to do the observed y-axis (response) variale minus the predicted y-axis (response) variale. About half the time this will results in a negative value. That is okay, it just means that the observed value was less than the predicted value (i.e., the observed point was below the best-fit line).</li>
