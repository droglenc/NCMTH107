...
1. A one-sample Z-test is required because a (i) quantitative variable (length) was measured, (ii) individuals from only one group/population were sampled (Chivyrkui Bay, Lake Baikal), and (iii) &sigma; is known (given in the background).
...
1. The true mean length of all pike in Lake Baikal appears to be greater than 600 mm. (*You strictly tested for a difference, but the x&macr; clearly shows that the mean length is greater than 600 mm.*)
1. One is 95% confident that the true mean length of all pike in Lake Baikal is between 609.6 and 702.6 mm (from 656.1 &plusmn; 1.96*23.74, 656.1 &plusmn; 46.53). This further supports that the mean length of Lake Baikal pike is greater than 600 mm.
...
1. A one-sample Z-test is required because (i) a quantitative variable (length of pain) was measured, (ii) individuals from only one group/population were sampled (patients with this syndrome), and (iii) &sigma; is known (given in the background).
...
1. One is 90% confident that the mean duration of pain for all patients is more than `r formatC(pain.z$conf.int[1],format="f",digits=2)` years (`r tabs("Pain",display="cite")`), which further supports (just barely) that the mean is greater than 2.5.
...
1. A one-sample Z-test is required because (i) a quantitative variable (cholesterol level) was measured, (ii) individuals from only one group/population were sampled (Asian women aged 21-40 that had recently immigrated to the U.S.), and (iii) &sigma; is known (given in the background).
...
1. One is 90% confident that the mean cholesterol level for all Asian women aged 21-40 that recently immigrated to the U.S. is between `r formatC(ch.z$conf.int[1],format="f",digits=2)` and `r formatC(ch.z$conf.int[2],format="f",digits=2)` mg/dl (`r tabs("Cholest",display="cite")`), which further supports (just barely) that the mean is not different from 190 mg/dl.
