...
As is typical, the categorical variables are recorded as character type. Both `degree` and `grnsol` are **ordinal** categorical variables and, thus, should be converted to factor variables types so that the order of the levels can be controlled. To assist with this, use `unique()` to see the exact spelling of each of the levels.

<aside>
Ordinal variables should be converted to factor variables in R.
...
## Percentage Tables
Percentage tables are constructed in R by submitting the saved `xtabs()` object to `percTable()`.^[`percTable()` is from `NCStats`.] A total- or table-percentage table is constructed by default.
