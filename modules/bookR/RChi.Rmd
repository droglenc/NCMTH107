...
**IF** the raw data do NOT exist, but a summary table is provided, then those summary statistics must be entered into an object using `matrix()`. matrix. The summarized frequencies must first be entered into a vector with the first **COLUMN** of values followed by the second COLUMN and so on. This vector is then the first argument to `matrix()`, which will also include the number of columns in the frequency table in `ncol=`. For example, the same observed table from above would be constructed as follows.
...
1. p-value=`r kPvalue(ai.chi$p.value,include.p=FALSE) (Table \@ref(tab:ChiAIchi)).
...
**IF** the raw data do NOT exist, but a summary table is provided, then those summary statistics must be entered into an object using `c()`. Each item can be named in c() as shown below, which wiill make the results easier to interpret. For example, the same observed table from above would be constructed as follows.
...
## Expected Table
The R function to perform a Goodness-of-Fit test described in the next section requires a vector of expected proportions in each groups defined by the theoritical distribution in the hypotheses. This vector does not actually have to be proportionts, per se, but it must be values that are directly proportional to the expected proportions. Thus, the vector can contain the proportions, percentages, actual expected values, or any other set of values that are related to the expected proportions by a constant.
...
The results from \R{chisq.test()} should be assigned to an object so that useful information can be extracted.

```{r}
( sf.gof <- chisq.test(sf.obs,p=exp.p,rescale.p=TRUE,correct=FALSE) )
```

From the results above the observed table for Step 6 was shown further above, the &chi;<sup>2</sup> test statistic for Step 7 is `r formatC(sf.gof$statistic,format="f",digits=3)`, and the p-value for Step 8 is `r formatC(sf.gof$p.value,format="f",digits=8)` (with `r sf.gof$parameter` df).

Rejecting H<sub>0</sub> in a Goodness-of-Fit test simply indicates that the observed distribution differs from the theoretical distribution, but it is not clearly exactly where the difference occurs. The following code can be used to neatly put the observed values, expected values, and Pearson residuals next to each other, which may be helpful for explaining any differences that were observed.
