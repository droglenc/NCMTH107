...
knitr::include_graphics("https://pbs.twimg.com/profile_images/1041703659984191489/qWBxildv.jpg")
```
...
peng <- read.csv("penguins.csv") #BREAK
str(peng) #BREAK
peek(peng) #BREAK
```
...
`r chunk_reveal("filterBM",break_type="user",widths=c(35,65,0),title="## Isolate Variable")`
...
#BREAK
peng_chingen <- filterD(peng,species %in% c("Chinstrap","Gentoo")) #BREAK
peek(peng_chingen) #BREAK
```
...
#BREAK
peng_chinmale <- filterD(peng,species=="Chinstrap",sex=="male") #BREAK
peek(peng_chinmale) #BREAK
```
...
#BREAK
peng_heavy <- filterD(peng,body_mass_g>5900) #BREAK
peek(peng_heavy) #BREAK
```
...
#BREAK
peng_wt <- filterD(peng,body_mass_g>4500,body_mass_g<5000) #BREAK
peek(peng_wt) #BREAK
```
...
#BREAK
Summarize(~flipper_length_mm,data=peng_chin,digits=1) #BREAK

## Distribution of bill lengths for Chinstrap Penguins
#BREAK
Summarize(~bill_length_mm,data=peng_chin,digits=2) #BREAK
```
...
## Distribution of flipper lengths for Chinstrap Penguins
#BREAK1
ggplot(data=peng_chin,mapping=aes(x=flipper_length_mm)) +#BREAK2
...
## Distribution of bill lengths for Chinstrap Penguins
ggplot(data=peng_chin,mapping=aes(x=flipper_length_mm)) + #ROTATE
ggplot(data=peng_chin,mapping=aes(x=bill_length_mm)) + #ROTATE
...
#BREAK1
## Species composition
ggplot(data=peng,mapping=aes(x=species)) + #BREAK2
  geom_bar( #BREAK3
...
## Sampling by Island
ggplot(data=peng,mapping=aes(x=species)) + #ROTATE
ggplot(data=peng,mapping=aes(x=island)) + #ROTATE
...
## Distribution of flipper lengths by species
ggplot(data=peng,mapping=aes(x=species,y=flipper_length_mm)) + #BREAK2
...
ggplot(data=peng,mapping=aes(x=species,y=flipper_length_mm)) + #ROTATE
ggplot(data=peng,mapping=aes(x=species,y=bill_length_mm)) + #ROTATE
...
#BREAK1
ggplot(data=peng_chin,mapping=aes(x=flipper_length_mm,y=body_mass_g)) + #BREAK2
  geom_point( #BREAK3
    pch=21, #BREAK4
    color="black",fill="lightgray" #BREAK5
  ) + #BREAK3
...
ggplot(data=peng_chin,mapping=aes(x=flipper_length_mm,y=body_mass_g)) + #ROTATE
ggplot(data=peng,mapping=aes(x=flipper_length_mm,y=bill_depth_mm)) + #ROTATE
...
#BREAK
( slr <- lm(body_mass_g~flipper_length_mm,data=peng_chin) ) #BREAK
rSquared(slr)
```
...
```{r RegressionPredict, include=FALSE}
slr
...
`r chunk_reveal("RegressionPredict",break_type="user",widths=widths,title="## Make a Prediction")`
...
#BREAK1
t.test(~flipper_length_mm,data=peng_chin #BREAK2
       ,mu=190 #BREAK3
       ,alt="greater" #BREAK4
...
##   than Gentoo Penguins. Use alpha=0.10.
#BREAK1
t.test(flipper_length_mm~species,data=peng_chingen #BREAK2
       ,var.equal=TRUE #BREAK3
       ,alt="less" #BREAK4
       ,conf.level=0.90 #BREAK5
       ) #BREAK2
```
...
`r chunk_reveal("gof",break_type="user",widths=widths,title="## Goodness-of-Fit Test")`
