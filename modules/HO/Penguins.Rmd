...
```{r packages, include=FALSE, message=TRUE, warning=TRUE}
#!# Loading important packages ... must do every time
library(NCStats)
library(ggplot2)
```

`r chunk_reveal("packages",break_type="auto",widths=widths,title="# Load Important Packages")`
...
- Size (bill, flipper, weight) of three species of Penguins from three islands in Antarctica.
...
1. Download or save data (CSV file) to your computer.

--
1. Start an R script with code for important packages.

--
1. Save script in same folder as data.
...
`r chunk_reveal("filter1",break_type="auto",widths=widths)`
...
`r chunk_reveal("filter2",break_type="user",widths=widths)`
...
`r chunk_reveal("filter3",break_type="user",widths=widths)`
...
`r chunk_reveal("filterHvy",break_type="user",widths=widths)`
...
`r chunk_reveal("filterBM",break_type="user",widths=c(35,65,0))`
...
`r chunk_reveal("UnivEDAQsum",break_type="user",widths=widths)`
...
- First, use `ggplot()` to declare data.
  - Must include data.frame name in `data=` argument.
  - Set `x=` to variable names in `mapping=aes()`.

--
- Second, add on `geom_histogram()` with
...
`r chunk_reveal("UnivEDAQhist",break_type="non_seq",widths=widths)`
...
`r chunk_reveal("UnivEDACsum",break_type="user",widths=widths)`
...
`r chunk_reveal("UnivEDACsump",break_type="user",widths=widths)`
...
`r chunk_reveal("UnivEDACbar",break_type="non_seq",widths=widths)`
...
- First, make histogram exactly as before.
...
--
- Second, add on `geom_boxplot()` with
...
`r chunk_reveal("BivEDAQsum",break_type="user",widths=widths)`
...
`r chunk_reveal("BivEDAQPlot",break_type="non_seq",widths=widths)`
...
percTable(freq2) #BREAK

## Row percentages table
percTable(freq2,margin=1) #BREAK
...
`r chunk_reveal("RegressionSum",break_type="user",widths=widths)`

---

## Showing Best-Fit Line

- First make a scatterplot as usual

--
- Second add line with `geom_smooth(method="lm",se=FALSE)`
...
`r chunk_reveal("RegressionPlot",break_type="non_seq",widths=widths)`
...
`r chunk_reveal("ttest1",break_type="non_seq",widths=widths)`
...
`r chunk_reveal("LevenesTest",break_type="user",widths=widths)`
...
`r chunk_reveal("ttest2",break_type="non_seq",widths=widths)`
...
- First, use `xtabs()` to make a two-way observed frequency table.
  - Grouping variable should be in rows.
  - Response variable should be in columns.

--
- Second, compute chi-square results with `chisq.test()`
...
`r chunk_reveal("chi",break_type="user",widths=widths)`
