...
\section{Predictions}
Once a best-fit line has been identified (criteria for doing so is discussed in \sectref{sect:BestFitLine}), the equation of the line can be used to predict the average value of the response variable for individuals with a particular value of the explanatory variable. For example, the best-fit line for the mercury data shown in \figref{fig:HGpredict} is
...
slp <- formatC(coef(lm.chimp)[2],format="f",digits=2)
int <- formatC(coef(lm.chimp)[1],format="f",digits=2)
...
ggplot(data=chimp,mapping=aes(x=huntparty,y=percsucc)) +
  ...
  stat_SLR_equation(aes(label=..rr.label..),
...
\begin{QAlist}
  ...
  \begin{QAlist}
    \item The predicted hunt success for parties with 12 individuals is \Sexpr{int} + \Sexpr{slp}*12 = 68.7\%.
  \end{QAlist}
  \item What is the residual if the hunt success for 10 individuals is 50\%?
  \begin{QAlist}
    \item The residual in this case is $50$-(\Sexpr{int} + \Sexpr{slp}*10) = $50$-61.3 = -11.3. Therefore, it appears that the success of this hunting party is 11.3\% lower than average for this size of hunting party.
  \end{QAlist}
...
int <- formatC(coef(lm2)[1],format="f",digits=1)
r2 <- formatC(rSquared(lm2),format="f",digits=2)
ggplot(data=cars93,mapping=aes(x=Weight,y=HMPG)) +
  geom_point(pch=21,color="black",fill="darkgray",size=2) +
  geom_smooth(method="lm",se=FALSE,color="red",size=1) +
  stat_SLR_equation(aes(label=..eq.label..),
                    label.x.npc=0.45,label.y.npc=1,cf.digits=c(1,4)) +
  stat_SLR_equation(aes(label=..rr.label..),
                        label.x.npc=0.45,label.y.npc=0.90) +
...
\begin{QAlist}
  ...
  \begin{QAlist}
    \item The predicted highway MPG for a car that weighs 3100 lbs is \Sexpr{int} - \Sexpr{slp}(3100) = 29.0 MPG.
  \end{QAlist}
  ...
  \begin{QAlist}
    \item The predicted highway MPG for a car that weighs 3500 lbs is \Sexpr{int} - \Sexpr{slp}(3500) = 26.1. Thus, the residual for this car is 24 - 26.1 = -2.1. Therefore, it appears that this car gets 2.5 MPG LESS than an average car with the same weight.
  \end{QAlist}
