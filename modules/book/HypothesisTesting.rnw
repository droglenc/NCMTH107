...
Even though $\beta$ can not usually be calculated, a researcher can make decisions that will reduce $\beta$ \figrefp{fig:SLBetaRelations}. For example, a researcher can decrease $\beta$ by increasing $\alpha$ or $n$. Increasing $n$ is more beneficial because it does not result in an increase in Type I errors as would occur with increasing $\alpha$.

In addition, $\beta$ increases as the difference between the hypothesized mean ($\mu_{0}$) and the actual mean ($\mu_{A}$) decreases \figrefp{fig:SLBetaRelations}. This means that you will make more Type II errors when the hypothesized and actual mean are close together. In addition, $\beta$ increases with a increasing amount of natural variability (i.e., $\sigma$; \figref{fig:SLBetaRelations}). In other words, more Type II errors are made when there is more variability among individuals. A researcher cannot control the difference between $\mu_{0}$ and $\mu_{A}$ or the value of $\sigma$. However, it is important to know that if a situation with a ``large'' amount of variability is encountered or the difference to be detected is small, the researcher will need to increase $n$ to reduce $\beta$. For example, if $n$ could be doubled in the Square Lake example to 100, then $\beta$ (for $H_{0}:\mu=105$) would decrease to approximately 0.18 \figrefp{fig:SLBetaRelations}.

<<SLBetaRelations, echo=FALSE, fig.width=6.5, fig.height=6.5, out.width='.7\\linewidth', fig.cap="The relationship between one-tailed (lower) $\\beta$ and $\\alpha$, $n$, actual mean ($\\mu_{A}$), and $\\sigma$. In all situations where the variable does not vary, $\\mu_{0}=105$, $\\mu_{A}=98.06$, $\\sigma=31.49$, $n=50$, and $\\alpha=0.05$. ">>=
