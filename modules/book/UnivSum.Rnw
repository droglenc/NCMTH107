...
<<results='asis', echo=FALSE>>=
mco <- mc[order(mc)]
mcx <- xtable(matrix(mco,nrow=2,byrow=TRUE),digits=0)
print(mcx,floating=FALSE,include.rownames=FALSE,include.colnames=FALSE,hline.after=NULL)
@
...
<<MineHist1, echo=FALSE, fig.width=6, fig.height=6, out.width='.7\\linewidth', fig.cap="Steps (described in text) illustrating the construction of a histogram.">>=
...
<<histOF, echo=FALSE, fig.cap='Histogram of length of eruptions for Old Faithful geyser with varying number of bins/classes.', fig.width=10.5, fig.height=3.5, out.width='.95\\linewidth'>>=
...
<<MTH107BarChart, echo=FALSE, fig.cap="Bar charts of the frequency of individuals in MTH107 during Winter 2010 by whether or not they were required to take MTH107 (\\textbf{Left}) and year-in-school (\\textbf{Right}).", fig.width=7, fig.height=3.5, out.width='.65\\linewidth'>>=
tmp1 <- data.frame(x=c("N","Y"),y=c(30,38))
tmp2 <- data.frame(x=c("Fr","So","Jr","Sr"),y=c(19,12,39,9))

bc1 <- ggplot(data=tmp1,mapping=aes(x=x,y=y)) +
  ...
  theme_NCStats()
bc2 <- ggplot(data=tmp2,mapping=aes(x=x,y=y)) +
  geom_col(fill="gray80",color="black") +
