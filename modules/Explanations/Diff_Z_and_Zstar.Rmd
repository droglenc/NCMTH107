...
When applying the [11-steps of any Hypothesis Test](../11-steps) to a 1-Sample Z-test, you will compute a Z test statistic in Step 6 and Z$^{*}$ "critical value" when constructing a confidence region in Step 11. These two values both come from the standard normal (i.e., Z) distribution, but they are **NOT THE SAME** and, thus, it is important to understand how they differ.

The Z test statistic is computed with $Z=\frac{\bar{x}-\mu_{0}}{\frac{\sigma}{\sqrt{n}}}$ and is a measure of how many standard errors (i.e., $\frac{\sigma}{\sqrt{n}}$) the sample mean ($\bar{x}$) is from the hypothesized population mean ($\mu_{0}$). As [shown here](Calc_pvalue_Z) its actual calculation depends on what "more extreme" means from H<sub>A</sub>. It is ultimately used to compute the p-value so that (with &alpha;) you can make a decision about H<sub>0</sub>.

Z$^{*}$ is a critical value that determines how many standard errors wide a confidence region should be so that it will contain the parameter (e.g., $\mu$) the confidence level (e.g., 90, 95, or 99%) of times. As [shown here](Calc_CI_Z) Z$^{*}$ depends on the level of confidence and what "more extreme" means from H<sub>A</sub>.

<div class="alert alert-warning">
<strong>Note:</strong> 
<ul>
<li>The Z test statistic is always computed from $Z=\frac{\bar{x}-\mu_{0}}{\frac{\sigma}{\sqrt{n}}}$. It will be used in a FORWARD calculation to find the p-value (i.e., the area "more extreme" of Z).</li>
<li>Z$^{*}$ is always computed from a REVERSE calculation (i.e., the Z with the level of confidence "more extreme").</li>
</ul>
</div>
...
## First Example
Suppose that I hypothesized that the population mean time to take a quiz is less than 100 minutes and that, in a sample of 30 students, the sample mean time to take the quiz was 95.2 minutes. Further, suppose that &sigma;=20 and &alpha;=0.10. For simplicity below, note that the SE is $\frac{20}{\sqrt{30}}$=3.651 and the level of confidence is 100(1-0.10)=90%.
...
This p-value is less than &alpha; so we REJECT H<sub>0</sub> and conclude that it does take significantly less than a mean of 100 minutes for ALL students to take the quiz.

In this case we will construct a 90% upper confidence bound for &mu;. Thus, Z$^{*}$ is the Z that has 90% GREATER (i.e., the direction of H<sub>A</sub>) and is `r formatC(distrib(0.90,type="q"),format="f",digits=3)` as computed with
```{r}
...
<div class="alert alert-warning">
In this example, note how the Z test statistic (-1.315) is not related at all to Z$^{*}$ except that they both are from a Z distribution (mean=0, sd=1) and the shaded area was in the same direction (because both use H<sub>A</sub>). Make note of this in the next two examples as well.
</div>

----

## Second Example
Suppose that I hypothesized that the population mean time to walk between the Science Center and the Ponzio Center is more than 7 minutes and that, in a sample of 50 students, the sample mean time to make this walk was 8.3 minutes. Further, suppose that &sigma;=1.3 and &alpha;=0.05 so that SE=$\frac{1.3}{\sqrt{50}}$=0.184 and the level of confidence is 95%.
...
The 95% lower confidence bound uses Z$^{*}$=`r formatC(distrib(0.95,type="q",lower.tail=FALSE),format="f",digits=3)` as found with
```{r}
...
The Z test statistic is $\frac{33.8-30}{1.534}$=2.477 which results in a HALF p-value of `r formatC(distrib(2.477,lower.tail=FALSE),format="f",digits=4)`, a whole p-value of `r formatC(2*distrib(2.477,lower.tail=FALSE),format="f",digits=4)`, and NOT rejecting H<sub>0</sub>.
```{r}
distrib(2.477,lower.tail=FALSE)
```

The 99% confidence interval uses Z$^{*}$=&plusmn;`r formatC(distrib(0.995,type="q"),format="f",digits=3)` as shown below.
```{r}
