...
Z$^{*}$ is a critical value that determines how many standard errors wide a confidence regions should be so that it will contain the parameter (e.g., $\mu$) the confidence level (e.g., 90, 95, or 99%) of times. As [shown here](Calc_CI_Z) Z$^{*}$ depends on the level of confidence and what "more extreme" means from H<sub>A</sub>.
...
In this case we will constuct a 90% upper confidence bound for &mu;. Thus, Z$^{*}$ is the Z that has 90% UPPER (i.e., the direction of H<sub>A</sub>) and is `r formatC(distrib(0.90,type="q"),format="f",digits=3)` as computed with
```{r}
...
The 95% lower confidence bound uses Z$^{*}$=`r formatC(distrib(0.9,type="q",lower.tail=FALSE),format="f",digits=3)` as found with
```{r}
