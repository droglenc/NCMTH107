...
<<NormDensityEx, echo=FALSE, fig.width=10.5, out.width='.95\\linewidth', fig.cap="Depiction of fitting a smooth curve to a histogram to serve as a model for the distribution.">>=
...
<<NormMultDists, echo=FALSE, fig.width=7, out.width='.8\\linewidth', fig.cap="Nine normal distributions. Distributions with the same line type have the same value of $\\mu$ (solid is $\\mu$=0, dashed is $\\mu$=2, dotted is $\\mu$=5). Distributions with the same color have the same value of $\\sigma$ (black is $\\sigma$=0.5, red is $\\sigma$=1, and green is $\\sigma$=2).">>=
...
<<NormEmpiricalRule, echo=FALSE, fig.width=5, fig.height=4, out.width='.5\\linewidth', fig.cap="Depiction of the 68-95-99.7 (or Empirical) Rule on a normal distribution.">>=
...
<<NormEmpiricalRuleCalc, echo=FALSE, fig.cap="The N(3,1) distribution depicting how the 68-95-99.7 Rule is used to compute the percentage of individuals with values greater than 5.">>=
...
For example, suppose that the heights of a population of students is known to be $H\sim N(66,3)$. The proportion of students in this population that have a height less than 71 inches is computed below. Thus, approximately \Sexpr{formatC(pnorm(71,mean=66,sd=3)*100,format="f",digits=1)}\% of students in this population have a height less than 71 inches \figrefp{fig:NormZCalc1}.
...
The proportion of students in this population that have a height \textit{greater} than 68 inches is computed below (note use of \R{lower.tail=FALSE}). Thus, approximately \Sexpr{formatC(pnorm(68,mean=66,sd=3,lower.tail=FALSE)*100,format="f",digits=1)}\% of students in this population have a height greater than 68 inches \figrefp{fig:NormZCalc2}.
...
For example, the area between 62 and 70 inches of height is found below. Thus, \Sexpr{formatC((pnorm(70,mean=66,sd=3)-pnorm(62,mean=66,sd=3))*100,format="f",digits=1)}\% of students in this population have a height between 62 and 70 inches.

<<NormZCalc3, par1a=TRUE, fig.cap="Calculation of the areas less than 70 inches (\\textbf{Left}) and 62 inches (\\textbf{Right}).",fig.show='hide'>>=
