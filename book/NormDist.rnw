...
popn <- rnorm(10000)
br <- seq(-4,4,0.5)
hist(popn,probability=TRUE,breaks=br,xlab="Measurement",ylab="",yaxt="n",main="",col="gray90")
hist(popn,probability=TRUE,breaks=br,xlab="Measurement",ylab="",yaxt="n",main="",col="gray90")
x <- seq(-4,4,0.001)
...
hist(popn,probability=TRUE,breaks=br,xlab="Measurement",ylab="",yaxt="n",main="")
...
x <- seq(-4,4,0.001)
plot(x,dnorm(x),type="l",lwd=3,col="red",xlab="Measurement",ylab="",yaxt="n",xlim=c(-4,4))
x.shade <- x[x>=0 & x<=2]
...
@
Thus, approximately \Sexpr{formatC(pnorm(71,mean=66,sd=3)*100,format="f",digits=1)}\% of the students in this population have a height less than 71 inches.  The proportion of students in this population that have a height \textit{greater} than 68 inches is computed by including the \R{lower.tail=FALSE} argument as follows (with the results shown below and in \figref{fig:NormZCalc2}),
...
@
Thus, approximately \Sexpr{formatC(pnorm(68,mean=66,sd=3,lower.tail=FALSE)*100,format="f",digits=1)}\% of the students in this population have a height greater than 68 inches.
...
<<NormZCalc3, echo=-1, par1a=TRUE, fig.cap="Calculation of the areas less than 70 inches (\\textbf{Left}) and 62 inches (\textbf{Right}).">>=
( AB <- distrib(70,mean=66,sd=3) )
( A <- distrib(62,mean=66,sd=3) )
AB-A
@

Thus, \Sexpr{formatC((pnorm(70,mean=66,sd=3)-pnorm(62,mean=66,sd=3))*100,format="f",digits=1)}\% of students in this population have a height between 62 and 70 inches.
...
Thus, 20\% of the population of students is shorter than \Sexpr{formatC(qnorm(0.20,mean=66,sd=3),format="f",digits=1)} inches.  ``Greater than'' reverse questions are computed by including the \R{lower.tail=FALSE} argument.  For example, the top 10\% of heights \figrefp{fig:NormZCalc5} is found with
