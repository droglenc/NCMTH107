...
gss2 <- read.csv("data/GSSEnviroQues.csv")
...
headtail(mc)
@
      \item The populations are independent (cartons could not be both linead and unlined).  The sample size ($n_{L}+n_{UL}$=\Sexpr{dim(mc)[1]}) is $>40$.  Therefore, the test statistic computed below should reasonably follow a t-distribution with $n_{L}+n_{UL}-2$ = \Sexpr{dim(mc)[1]}-2 = \Sexpr{dim(mc)[1]-2} df.  The variances appear to be UNequal because the Levene's test p-value (\Sexpr{kPvalue(levenesTest(dioxin~type,data=mc)[1,"Pr(>F)"])}) is larger than $\alpha$.  The Levene's test was computed with
<<>>=
...
headtail(mg)
@
      \item The populations are independent (no classes consisted of all the same students).  The sample size ($n_{m}+n_{o}$=\Sexpr{dim(mg)[1]}) is $>40$.  Therefore, the test statistic computed below should reasonably follow a t-distribution with $n_{m}+n_{o}-2$ = \Sexpr{dim(mg)[1]}-2 = \Sexpr{dim(mg)[1]-2} df.  The population variances appear to be equal because the Levene's test p-value (\Sexpr{kPvalue(levenesTest(gpa~class.type,data=mg)[1,"Pr(>F)"])}) is larger than $\alpha$.  The Levene's test was computed with
<<>>=
...
@
<<echo=FALSE,results='hide'>>=
tmp <- levenesTest(hrs.worked~city,data=med)
@

Therefore, the test statistic computed below should reasonably follow a t-distribution with $n_{1}+n_{2}-2$ = \Sexpr{dim(med)[1]}-2 = \Sexpr{dim(med)[1]-2} df.  The population variances appear to be equal because the Levene's test p-value (\Sexpr{kPvalue(tmp[1,"Pr(>F)"])}) is larger than $\alpha$.  The Levene's test was computed with
<<>>=
...
str(d)
@
      \item The populations are independent because the individual cannot both be a donor and a non donor.  The sample size ($n_{o}+n_{n}$=\Sexpr{dim(d)[1]}) is $>40$.  Therefore, the test statistic computed below should reasonably follow a t-distribution with $n_{o}+n_{n}-2$ = \Sexpr{dim(d)[1]})-2 = \Sexpr{dim(d)[1]-2}) df.  The variances appear to be equal because the Levene's test p-value (\Sexpr{kPvalue(levenesTest(DAS~donor,data=d)[1,"Pr(>F)"])}) is larger than $\alpha$.  The Levene's test was computed with
<<>>=
