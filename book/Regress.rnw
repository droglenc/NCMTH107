...
fitPlot(lm.lum,pch=16,xlab="Highway MPG",ylab="City MPG",main="")
...
y1 <- slp*x+int+e1
lm1 <- lm(y1~x)
fitPlot(lm1,xlab="Explanatory",ylab="Response",xaxt="n",yaxt="n",main="")
mtext("Assumptions Met",line=0.25,col="blue")

e2 <- ((x-mean(x))/5)^2 + rnorm(n,0,sigma/4)
y2 <- slp*x+int+e2
lm2 <- lm(y2~x)
fitPlot(lm2,xlab="Explanatory",ylab="Response",xaxt="n",yaxt="n",main="")
...
y3 <- slp*x1+int+e3
lm3 <- lm(y3~x1)
fitPlot(lm3,xlab="Explanatory",ylab="Response",xaxt="n",yaxt="n",main="")
mtext("Heteroscedastic",line=0.25,col="red")

y4 <- 2*log(x1) + rnorm(n,0,0.4)
y4 <- exp(y4)
lm4 <- lm(y4~x1)
fitPlot(lm4,xlab="Explanatory",ylab="Response",xaxt="n",yaxt="n",main="")
...
sbp1 <- read.table("data/sbp.txt",header=TRUE)
lm.sbp <- lm(sbp~age,data=sbp1)
fitPlot(lm.sbp,pch=16,xlab="Age (yrs)",ylab="Systolic Blood Pressure",main="")
r2 <- rSquared(lm.sbp)
...
tv <- read.table("data/tv.txt",header=TRUE)
lm.tv <- lm(audience~rating,data=tv)
fitPlot(lm.tv,pch=16,xlab="Rating (%)",ylab="Audience (1000s)",main="")
...
molt <- read.table("data/molt.txt",header=TRUE)
out <- molt[33,]
molt1 <- molt[-33,]
lm.molt <- lm(duration~date,data=molt1)
fitPlot(lm.molt,pch=16,xlab="Start Date (d since July 1)",ylab="Duration (d))",main="")
...
d2 <- read.table("data/deer2.txt",header=TRUE)
lm.d2 <- lm(fawns~bodymass,data=d2)
fitPlot(lm.d2,pch=16,xlab="Doe Body Mass (kg)",ylab="Mean Fawns per Doe",main="",ylim=c(1,4))
...
\begin{exsection}
  ...
  \item \label{revex:RegRIFA} \rhw{} \cite{Allenetal1997} investigated the impact of the density of red-imported fire ants (\emph{Solenopsis invicta}; RIFA) on the recruitment of white-tailed deer (\emph{Odocoileus virginianus}) fawns (an index of does to fawns).  A modified version of their results are found in \href{https://sites.google.com/site/ncstats/data/rifa.txt}{rifa.txt}.  Load these data into R and produce results that can be used to answer the questions below. \ansref{ans:RegRIFA}
    \begin{Enumerate}
      \item What is the response variable?
      \item What is the explanatory variable?
      \item In terms of the variables of this problem, what is the equation of the best-fit line?
