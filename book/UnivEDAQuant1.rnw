...
\chapter[Summary Single Quant]{Summaries for Single Quantitative Variable)} \label{chap:UnivEDAQuant1}
...
\lettrine{S}{ummarizing large quantities of data with} few graphical or numerical summaries makes it is easier to identify meaning from data (disccused in \modref{chap:WhyStatsImportant}). Numeric and graphical summaries specific to a single quantitative variable are described in this module. Interpretations from these numeric and graphical summaries are described in the next module.
...
\subsection{Mean}
The mean is the arithmetic average of the data. The sample mean is denoted by $\bar{x}$ and the population mean by $\mu$. The mean is simply computed by adding up all of the values and dividing by the number of indivuals. If the measurement of the generic variable $x$ on the $i$th individual is denoted as $x_{i}$, then the sample mean is computed with these two steps,
\begin{Enumerate}
...
\subsection{Numerical Summaries in R} \label{sect:DescStatsCenter}
...
<<MineHist1, echo=FALSE, fig.width=10.5, out.width='.95\\linewidth', fig.cap="Steps illustrating the development of a histogram.">>=
...
<<MineHist2, echo=FALSE, fig.cap="Histogram of number of open pit mines.">>=
