...
\vspace{-12pt}
  \begin{Enumerate}
    ...
    \item The expected frequency in each of the four cells is greater than five \tabrefp{tab:ChiTreeObs}.
    ...
    \item p-value=\Sexpr{kPvalue(pchisq(4.927,df=1,lower.tail=FALSE),include.p=FALSE)}.
...
The 11-steps \sectrefp{sec:11Steps} for a hypothesis test for this situation are as follows:
  \begin{Enumerate}
    ...
    \item $\chi^{2}$=\Sexpr{formatC(fp.chi$statistic,format="f",digits=0)} with \Sexpr{fp.chi$parameter} df \tabrefp{tab:ChiFPchi}.
