...
<<chiDist, echo=FALSE, cache=TRUE, fig.show='animate', fig.cap='$\\chi^2$ distributions with varying degrees-of-freedom.', aniopts='controls,loop,autoplay'>>=
...
\begin{exsection}
  ...
  \item \label{revex:ChiGOFOtterMortality} \rhw{} \cite{Philcoxetal1999} examined patterns in the road-related mortalities of otters (\emph{Lutra lutra}) in Britain from 1971 to 1996.  One aspect of their analysis was to examine the sex ratio of road-killed otters.  The sex of all otters for which sex could be identified are recorded in \href{https://sites.google.com/site/ncstats/data/OtterMort.txt}{OtterMort.txt}.  Use these data to determine if there is a significant (at the 1\% level) bias in the sex ratio of road-killed otters. \ansref{ans:ChiGOFOtterMortality}
...
The 11-steps \sectrefp{sec:11Steps} for completing a full hypothesis test for the Christmas tree example presented at the beginning of this chapter are as follows:
  \begin{enumerate}
    ...
    \item The p-value is \Sexpr{kPvalue(pchisq(4.927,df=1,lower.tail=FALSE))}.
<<fig.show='hide'>>=
...
\begin{quote}
\textsl{Daniel Weiss (in ``100\% American'') reported the results of a survey of 300 first-time fathers from four different hospitals (labeled as A, B, C, and D).  Each father was asked if he was present (or not) in the delivery room when his child was born.  The results of the survey are in \href{https://sites.google.com/site/ncstats/data/FatherPresent.txt}{FatherPresent.txt}.  Use these data to determine if there is a difference, at the 5\% level, in the proportion of fathers present in the delivery room among the four hospitals.}
\end{quote}
...
The test statistic computed below should reasonably follow a $\chi^{2}$distribution, because there are at least five individuals in each cell of the expected table shown above.
    ...
    \item The test statistic is $\chi^{2}$=\Sexpr{formatC(fp.chi$statistic,format="f",digits=0)} with \Sexpr{fp.chi$parameter} df as seen with
<<>>=
fp.chi
@
    \item The p-value is \Sexpr{kPvalue(fp.chi$p.value)} as seen in the results above.
...
The test statistic computed below should reasonably follow a $\chi^{2}$distribution, because there are more than five individuals in each cell of the expected table shown above.
    ...
    \item The test statistic is $\chi^{2}$=\Sexpr{formatC(ai.chi$statistic,format="f",digits=2)} with \Sexpr{ai.chi$parameter} df as seen with
<<>>=
ai.chi
@
    \item The p-value is \Sexpr{kPvalue(ai.chi$p.value)} as seen in the results above.
...
\begin{exsection}
  ...
  \item \label{revex:ChiExer} \rhw{} USA Today presented two sets of data on why Americans don't exercise.  One set was for 1000 randomly selected men.  The other was for 1000 randomly selected women.  The results of the surveys are recorded in \href{https://sites.google.com/site/ncstats/data/Exercise.txt}{Exercise.txt}.  Determine, at the 1\% level, if the distribution of men and women differs among the six responses given. \ansref{ans:ChiExer}
  ...
  \item \label{revex:chiZoo1} \rhw{} The data in \href{}{Zoo1.csv} contains a list of animals found in several different zoos\footnote{These data are stored in a ``comma separated values'' (CSV) file rather than a ``tab delimited text'' file.  Thus, these data must be loaded into R with \R{read.csv()} rather than \R{read.table()}.  The arguments to \R{read.csv()} are the same as \R{read.table()}.}.  In addition, each animal was classified into broad ``type'' categories (``mammal'', ``bird'', and ``amph/rep'').  The researchers that collected these data wanted to examine if the distribution of broad animal types differed among zoos.  Test the researcher's question at the 5\% level \ansref{ans:chiZoo1}
