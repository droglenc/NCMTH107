...
\begin{Enumerate}
  ...
  \item Compute and interpret an appropriate confidence region for the parameter.
\end{Enumerate}
\vspace*{-6pt}

The order of some of these steps is arbitrary. However Steps 1-3 \textbf{MUST} be completed before collecting data (Step 4). Further note that Step 11 is completed to provide a more definitive statement about the value of the parameter when $H_{0}$ was rejected (i.e., if the parameter differs from the hypothesized value, then provide a range for which the actual parameter may exist).
...
\begin{minipage}{.75\textwidth}
  ...
  \vspace*{-6pt}
      ...
      \item \textbf{Assumptions:}
        ...
        \end{Enumerate}
      \item \textbf{Use with:} Quantitative response, one group (or population), $\sigma$ known.
  \end{Itemize}
\end{minipage}}
\end{table}
\vspace*{-3pt}
...
\begin{Enumerate}
  ...
  \item A 1-Sample Z-Test is required because (i) a quantitative variable (intra-class travel time) was measured, (ii) individuals from one group (or population) is considered (students at the Dean's school), and (iii) $\sigma$ is thought to be known (=1.60 mins).
  ...
  \item p-value=\Sexpr{kPvalue(pnorm(10.12,mean=10,sd=0.16,lower.tail=FALSE),include.p=FALSE)}.
  ...
  \item A 90\% lower confidence bound will use $Z{*}$=\Sexpr{formatC(qnorm(0.90,lower.tail=FALSE),format="f",digits=3)}. The lower confidence bound is thus $10.12-1.282*\frac{1.60}{\sqrt{100}}$=\Sexpr{formatC(10.12-1.282*1.60/sqrt(100),format="f",digits=2)}. Thus, I am 90\% confident that the mean intra-class travel time is more than \Sexpr{formatC(10.12-1.282*1.60/sqrt(100),format="f",digits=2)} minutes; further evidence that the mean intra-class travel time is not greater than 10 minutes.
\end{Enumerate}
...
\vspace*{-8pt}
\begin{quote}
\textsl{Machowiak et al. (1992) critically examined the belief that the mean body temperature is 98.6$^{o}$F by measuring body temperatures in a sample of healthy humans. Use their data in \href{https://raw.githubusercontent.com/droglenc/NCData/master/BodyTemp.csv}{BodyTemp.csv}, with $\sigma=0.63^{o}$F and $\alpha=0.01$ to determine if the mean body temperature differs from 98.6$^{o}$F.}
\end{quote}

<<echo=FALSE>>=
bt <- read.csv("https://raw.githubusercontent.com/droglenc/NCData/master/BodyTemp.csv")
@
<<1Zbtex, echo=FALSE, results='hide', fig.show='hide'>>=
...
\begin{Enumerate}
  ...
  \item A 1-Sample Z-Test is required because (i) a quantitative variable (i.e., body temperature) was measured, (ii) individuals from one group (or population) is considered (i.e., healthy humans), and (iii) $\sigma$ is thought to be known ($=0.63^{o}$F).
  ...
  \item (i) $n=\Sexpr{round(bt.z$parameter["n"],0)}\geq30$ and (ii) $\sigma$ is thought to be known ($=0.63^{o}$F).
  \item $\bar{x}$ = \Sexpr{round(bt.z$estimate,2)}$^{o}$F \tabrefp{tab:1Zbtex}.
  \item $Z=\Sexpr{round(bt.z$statistic,2)}$ \tabrefp{tab:1Zbtex}.
  \item p-value\Sexpr{kPvalue(bt.z$p.value,include.p=FALSE)} \tabrefp{tab:1Zbtex}.
  ...
  \item I am 99\% confident that the mean body temperature ($\mu$) for ALL healthy humans is between \Sexpr{round(bt.z$conf.int[1],1)} and \Sexpr{round(bt.z$conf.int[2],1)}$^{o}$F \tabrefp{tab:1Zbtex}.
\end{Enumerate}
