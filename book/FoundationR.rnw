...
str(iris)
@
In this example, it is seen that \Sexpr{kCounts(ncol(iris))} variables were recorded on \Sexpr{nrow(iris)} individuals.  The first four variables -- \var{\Sexpr{names(iris)[1]}}, \var{\Sexpr{names(iris)[2]}}, \var{\Sexpr{names(iris)[3]}}, and \var{\Sexpr{names(iris)[4]}} -- are various numerical measurements of sepals and petals.  The last variable -- \var{\Sexpr{names(iris)[5]}} -- is a factor variable that records the species of iris examined.  The levels of the \var{\Sexpr{names(iris)[5]}} variable may be seen by including this variable (must also include the data frame name) as the argument to \R{levels()}.
<<>>=
levels(iris$Species)
@
