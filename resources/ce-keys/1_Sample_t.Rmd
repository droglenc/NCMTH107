...
<ol>
<li>&alpha;=0.05.</li>
<li>H<sub>A</sub>: &mu;&#8800;178 and H<sub>0</sub>: &mu;=178, where $\mu$ is the mean number of CFU per 100 ml in ALL samples from this lake on this date.</li>
<li>1-sample t-test because (i) one population (this lake) was sampled, (ii) a quantitative variable (number of CFU per 100 ml) was recorded, and (iii) &sigma; is not known.</li>
...
<li>The sample mean ($\bar{x}$) is `r xbar` CFU per 100 ml.</li>
...
t.test(df$CFU,mu=178) # alt="two.sided" and conf.level=0.95 are the defaults
```
...
```{r CFU2Calc, echo=FALSE, results='hide', fig.show='hide'}
df <- read.csv("FecalColiform.csv")
...
```
`r figs("FChist")`
<li>The sample mean ($\bar{x}$) is `r formatC(fc.t$estimate,format="f",digits=0)` (`r tbls("FC",display="cite")`).</li>
...
kHtest(fc.t)
```
<li>The test statistic is Z=`r formatC(fc.t$statistic,format="f",digits=3)` with `r formatC(fc.t$parameter,format="f",digits=0)` df (`r tbls("FC",display="cite")`).</li>
<li>The p-value `r kPvalue(fc.t$p.value,include.p=FALSE,latex=FALSE)` (`r tbls("FC",display="cite")`).</li>
<li>The H<sub>0</sub> is rejected because the p-value<&alpha; (`r tbls("FC",display="cite")`).</li>
...
<li>I am 90\% confident that CFU per ALL 100 ml of water sampled from this lake on this date is greater than `r formatC(fc.t$conf.int[1],format="f",digits=0)` CFU per 100 ml (`r tbls("FC",display="cite")`).</li>
</ol>
